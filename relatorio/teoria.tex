\section{Backstepping - Formulação teórica com observador de ordem completa}
...
onde os parâmetros $a_2$, $a_1$, $a_0$ e $k_p$ são desconehcidos. Para esta
formulação apenas a saída do sistema $y$ está disponível, portanto $x_2$ e $x_3$
não são conhecidos e devem ser estimados. Podemos reescrever o sistema
\ref{eq:planta2}:
...
onde $\rho$ é estimativa de $\frac{1}{k_p}$. Os controles virtuais $\alpha_1$ e
$alpha_2$, a lei de controle $u$ e as leis de adaptação $\dot{\theta}$ e
$\dot{\rho}$ são obtidas pelo método de Lyapunov. A formulação e desenvolvimento
dos passos da solução do problema são realizados nas notas de aula. Dessa forma,
neste relatório fazemos o resumo das equações:
...
\tau__3 &= \tau_2 - \frac{\partial \alpha_2}{\partial y}\omega z_3\\
